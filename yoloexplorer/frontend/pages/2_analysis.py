...
DETECTION = (
    ...
    if INTEGRATION_IMPORT_ERROR is None
    else {}
)

SEGMENTATION = {} if INTEGRATION_IMPORT_ERROR is None else {}
...
def analyse_dataset():
    ...
    deci_ds = YoloFormatDetectionDataset(
        root_dir=info["path"],
...
def analysis():
    ...
    if INTEGRATION_IMPORT_ERROR:
        st.error(
            f"The following package(s) are required to run this module: `{INTEGRATION_IMPORT_ERROR}`. Please install them and try again."
        )
        return
