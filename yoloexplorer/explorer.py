...
SCHEMA = ["id", "img", "path", "cls", "labels", "bboxes", "segments", "keypoints", "meta"] # + "vector" with embeddings
...
class Explorer:
    ...
    def plot_imgs(self, ids=None, query=None, n=10, labels=True):
        ...
        resized_images = []
        df = self.sql(query) if query else self.table.to_pandas().set_index('id').loc[ids]
        if n < len(df):
            df = df[0:n]
    ...
    def plot_similirity_index(self, sim_thres=0.90, top_k=0.01, reduce=False, sorted=False):
        """
    ...
    def add_imgs(self, exp, idxs):
        ...
        self.table = self._create_table(self.temp_table_name, data=table_df, mode='overwrite')  # work on a temporary table
        self.log_status()
